...
export function createWolfxSource(opts = {}) {
  ...
  function handleDataFrame(raw, t) {
    ...
    if (candidates.length === 0) {
      // 速报空表是**正常**形态（当前没有速报数据），不算故障；预警的结构不对才算。
      if (isEqlist) { stats.lastFrames = 0; stats.lastAdded = 0; return }
      stats.errors += 1
    ...
    if (isEqlist) {
      ...
      if (md5 && md5 === lastEqlistMd5) { stats.lastFrames = 0; stats.lastAdded = 0; return }
    ...
    // 停更判定：用**最新事件的时刻**探中继（速报每天都有数据，所以"很久没有新事件"就是异常）
    if (staleAfterMs > 0 && stats.dataTime > 0) {
  ...
  function connect() {
    ...
    s.onmessage = (ev) => {
      ...
      if (!raw || typeof raw !== 'object') return
      if (raw.type === 'heartbeat') { heartbeatSeen += 1; return }
      try {
  ...
  function housekeeping() {
    ...
    if (isIdle(t)) {
      stats.idleSkips += 1
      stats.lastError = '超过 ' + idleMs + 'ms 无人使用 → 主动断开，等下次有人读时再连'
      closeSocket()
      scheduleNext(IDLE_RETRY_MS)
      return
    }
  ...
  return {
    ...
    markRead() {
      lastReadAt = now()
      // 本来因为无人使用而没在连 → 立刻建连，别让刚打开页面的用户白等一个空闲重探周期。
      if (running && !socket) scheduleNext(0)
    },
