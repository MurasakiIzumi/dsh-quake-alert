...
export function parseAtomEntries(xml) {
  const s = String(xml)
  if (s.indexOf('</entry>') === -1) return [] // 无闭合标签：直接判定为空，避免 O(n²) 回溯
...
export function createPoller(opts = {}) {
    ...
    for (const e of fresh) {
      ...
      try {
        ...
        if (singleStage) {
          ...
          const xml = await fetchText(e.detailUrl || e.id)
  ...
  function schedule(delay) {
    ...
    timer = setTimeout(async () => {
      ...
      schedule(skipped ? Math.min(intervalMs, IDLE_RETRY_MS) : intervalMs)
    }, delay)
    // 不要让轮询定时器把 DSH 进程钉住
