...
export const QuakeAlertSettingsSchema = z.object({
  ...
  disasters: z.object({
    ...
    cnRainstorm: z.boolean().default(true),
    cnGeology: z.boolean().default(true),
    ...
    overseasWeather: z.boolean().default(true),
  }),
  thresholds: z.object({
...
export function apply(ctx) {
  ctx.inject(['settings'], (settingsCtx) => {
    try {
      ...
      // settings.yaml 是**用户可编辑的机器级文件**，而这个 schema 不做类型强转：一个手写的
      // `notify: { volume: "0.5" }`（字符串）或 `tsunamiGrade: "Bogus"` 会让 register 本身抛错，
      // 命名空间因此根本没注册——客户端看到 status='unavailable'，静默退回 localStorage，
      // 用户手配的机器级配置全部不生效，而界面上只会多一行"配置存储：浏览器 localStorage"。
      // 兜住它并留下一条能查的日志：插件仍然可用（解析 / 匹配 / 提醒全在 Client），
      // 但"配置为什么没生效"必须能从日志里读到，而不是让人对着设置页猜（0.5.4）。
      try {
