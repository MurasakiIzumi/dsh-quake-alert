...
import { USGS_FEED_URL, NOAA_FEED_URL, parseUsgsEntries, parseNoaaEntries } from './global-sources.js'
    ...
    webCtx.effect(() => webCtx.webServer.register({
      ...
      handler: (req, res) => {
        try {
          ...
          // 源分派（0.4.0）：默认 jma，保持与旧版 Client 的兼容。用自有属性查找，
          // 避免 ?source=constructor 命中原型链后把 undefined 当 poller 用。
          const rawSource = String(url.searchParams.get('source') || 'jma').trim()
          const source = Object.prototype.hasOwnProperty.call(pollers, rawSource) ? rawSource : 'jma'
