 ...
 * 同形）——漏标一个的后果是"改这一项会重启 Host 半边"（重建四个轮询器 + 断开 Wolfx 常连）。
 * 回归里有一条断言专门守住漏标。
 *
...
export const QuakeAlertSettingsSchema = z.object({
  ...
  // 界面语言（0.8.1 先立字段，本地化本身在 0.9.0）。**必须是最后一个**：Host schema 的字段
  // 顺序要与 Client 的 DEFAULT_CFG 一致——"两侧默认值完全一致"那条断言是 JSON.stringify
  // 全量比较，顺序不同就会红。
  // 这里的白名单现在是单项；0.9.0 加语言包时与 Client 的 LANGUAGE_OPTIONS 一起扩。
  language: vol(z.union(['zh-CN']).default('zh-CN')),
})
