...
- 侧边栏底部一个圆点：**绿** = 全部正常 / **黄** = 连接中、重连中或链路降级 /
  ...
  **灰空心** = 用户关掉了。悬停可看到逐源明细。
- **设置 → 灾害预警 → 测试与诊断 → 源状态**：逐源的连接状态、增量条数、失败次数、增量缺口、最近拉取时间。
...
| 设置页「测试与诊断」里的「源状态」区块 | 逐源状态、增量、失败、缺口、最近拉取 | 0.4.1 可用（0.8.0 起该区块在「测试与诊断」下） |
...
| Client 侧只读诊断快照 | 设置页「测试与诊断」一键生成并复制：聚合状态、逐源状态与数据健康、增量计数、大陆源链路模式、**被跨源权威源压掉的条数**、关注点摘要（含每个点的来源分支 `origin`）、最近几条记录为什么没响铃。快照格式版本 2（0.8.0 起） | 0.5.0 可用，0.8.0 加 `authority` 段 |
