...
- Several DSH pages **share one persisted weather-alert cursor**: a telegram already handled by one page is not replayed in another (cross-tab de-duplication only ever lets one page announce it). The trade-off is that a page opened or reloaded later does not add those already-consumed entries to its own "Recent alerts" list.
...
```
client/src/*.js       # client sources: 17 standard ESM modules (explicit import/export; each header states job + deps)
...
lib/poller.js         # Host half: JMA Atom feed poller (entry de-duplication, ring buffer, cursor)
...
node tests/sync-test.cjs               # regression tests (439 assertions)
```
...
Current version **0.3.3**. See [CHANGELOG.md](./CHANGELOG.md) for the details of each release.
