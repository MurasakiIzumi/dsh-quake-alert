...
- All realtime logic runs in the browser (the Client half): the WebSocket connection, parsing, matching, notifications and the history list. The Host half registers the `quake-alert` settings namespace (machine-level `settings.yaml`) and serves the read-only municipality table at `/dsh-quake-alert/areas`.
...
node tests/sync-test.cjs               # regression tests (236 assertions)
```
...
Current version **0.2.2**. See [CHANGELOG.md](./CHANGELOG.md) for the details of each release.
