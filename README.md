...
Current version **0.5.4** (a review release: the display state is now composed in one place, so a
"data format error" blue dot can no longer be wiped out by a routine reconnect or by the staleness
probe; clearing the history now really clears it across tabs; JMA summary telegrams are no longer
labelled "special warning" unless they really are level 5; and the CI now runs the regression
suite, not just the contract self-check). The suite went from 1070 to 1103 assertions. See [CHANGELOG.md](./CHANGELOG.md) for the details of each release.
